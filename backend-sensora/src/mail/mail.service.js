...
var MailService = function () {
    ...
    var MailService = _classThis = /** @class */ (function () {
        ...
        MailService_1.prototype.isConfigured = function () {
        ...
        MailService_1.prototype.enviarEmail = function (_a) {
                    ...
                    switch (_c.label) {
                        case 0:
                            if (!this.isConfigured()) {
                                this.logger.warn('Envio de e-mail ignorado: RESEND_API_KEY/EMAIL_FROM não configurados neste ambiente.');
                                return [2 /*return*/];
                            }
                            _c.label = 1;
                        case 1:
                            _c.trys.push([1, 3, , 4]);
                            return [4 /*yield*/, fetch(RESEND_API_URL, {
                                    method: 'POST',
                                    headers: {
                                        Authorization: "Bearer ".concat(this.apiKey),
                                        'Content-Type': 'application/json',
                                    },
                                    body: JSON.stringify({ from: this.from, to: to, subject: subject, html: html }),
                                    signal: AbortSignal.timeout(RESEND_TIMEOUT_MS),
                                })];
                        case 2:
                            response = _c.sent();
                            if (!response.ok) {
                                this.logger.error("Falha ao enviar e-mail via Resend: HTTP ".concat(response.status));
                            }
                            return [3 /*break*/, 4];
                        case 3:
                            error_1 = _c.sent();
                            this.logger.error('Falha ao enviar e-mail via Resend', error_1 instanceof Error ? error_1.stack : String(error_1));
                            return [3 /*break*/, 4];
